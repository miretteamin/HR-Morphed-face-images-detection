...
def plot_det_curve(y_true: np.ndarray, y_pred: np.ndarray):
    ...
    plt.plot(bpcer_vals, macer_vals, marker='o', linestyle='-', color='b', label='DET Curve')
    plt.xlabel('BPCER')
    plt.ylabel('MACER')
...
def plot_macer_timeseries(macer_history, epochs):
    ...
    """
    plt.figure(figsize=(8, 6))
    plt.plot(epochs, macer_history, marker='s', linestyle='-', color='g', label='MACER@BPCER=1%')
    plt.xlabel('Epoch')
...
def plot_det_3d_evolution(y_true_list: list, y_pred_list: list, epoch_numbers: list):
    ...
    for i, (y_true, y_pred, epoch) in enumerate(zip(y_true_list, y_pred_list, epoch_numbers)):
        ...
        ax.plot(bpcer_vals, macer_vals, zs=epoch, zdir='z', label=f'Epoch {epoch}', alpha=0.7)
