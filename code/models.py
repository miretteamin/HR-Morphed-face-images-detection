...
class S2DCNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=16, kernel_size=3, stride=1, padding=1)  # Output: 16 x 64 x 96
        self.space_to_depth1 = SpaceToDepth(block_size=2)  # Reduces spatial size, increases depth
        self.conv2 = nn.Conv2d(in_channels=64, out_channels=32, kernel_size=3, stride=1, padding=1)  # Adjusted input channels
        self.space_to_depth2 = SpaceToDepth(block_size=2)
        self.conv3 = nn.Conv2d(in_channels=128, out_channels=64, kernel_size=3, stride=1, padding=1)  # Adjusted input channels
        
        # Fully connected layers
        self.fc1 = nn.Linear(64 * 4 * 6 * 16, 128)  # Match flattened feature size after conv3
        self.fc2 = nn.Linear(128, 1)  # Single logit output for binary classification

    def forward(self, x):
 
        x = F.relu(self.conv1(x))  # 16 x 64 x 96

        x = self.space_to_depth1(x)  # 64 x 32 x 48 (block_size=2)

        x = F.relu(self.conv2(x))  # 32 x 32 x 48

        x = self.space_to_depth2(x)  # 128 x 16 x 24 (block_size=2)
     
        x = F.relu(self.conv3(x))  # 64 x 16 x 24
   
        
        # Flatten and feed to fully connected layers
        x = x.view(x.size(0), -1)  # Flatten to [batch_size, 64 * 4 * 6]
